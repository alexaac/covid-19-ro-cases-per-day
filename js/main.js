...
(() => {
  ...
  const changeView = () => {
    let width =
        window.innerWidth <= 360 ? 600 : window.innerWidth <= 960 ? 1000 : 1200,
      height = (2 / 3) * width,
      radius =
        window.innerWidth <= 360 ? 0.5 : window.innerWidth <= 960 ? 1 : 1.5;
    ...
    d3.select('#filter-case').on('click', () => {
    ...
    const caseChanged = () => {
      var selection = [
        d3.select('#min-x').node().value,
        d3.select('#max-x').node().value,
      ];
